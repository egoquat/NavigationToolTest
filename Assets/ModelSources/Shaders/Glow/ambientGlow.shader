...
//				float2 uv2 : TEXCOORD1;
			...
			v2f vert (Input inp) {
				...
				outp.uv = inp.texcoord.xy + float2(0, (_ScreenParams.w-1));
/*				outp.uv2 = outp.uv;
			#if SHADER_API_D3D9
				if (_MainTex_TexelSize.y < 0)
					outp.uv2.y = 1-outp.uv2.y;
			#endif
*/			
				return outp;
			}
			...
			half4 frag (v2f inp) : COLOR {
				half4 mainMap = tex2D( _MainTex, inp.uv)*3;
//				half4 ambientMap = tex2D( _AmbiTex, inp.uv2)*3;

//				return mainMap + ambientMap;
				return mainMap;
			}
			ENDCG
		}
