...
//				eyeDir = mul(TangentSpace, eyeDir);
...
//				half3 normal = UnpackNormal(tex2D( _BumpTex, inp.uv)).rgb;
				...
				float specularAttn = saturate(pow( saturate(dot(normal, inp.halfVector)), 16)-0.1);
				half specularMap = tex2D( _SpecTex, inp.uv).r;
				half4 ambientMap = tex2D( _AmbiTex, inp.uv);
				float ambi = saturate(ambientMap.r+ambientMap.g+ambientMap.b);

				return (tex2D (_MainTex, inp.uv) * diffuseAttn) + specularAttn/* * (specularMap+0.1)*/ + ambi;//entMap;//*2;// * 2 for the worst specluarMap
			}
			ENDCG
		}
	}
